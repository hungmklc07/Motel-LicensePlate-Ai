...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(`
            INSERT INTO vehicles_in (license_plate, building_id, image_path)
            VALUES ($1, $2, $3)
            RETURNING *
        ...
        console.error('Lỗi ghi nhận xe vào:', err);
        res.status(500).json({ message: 'Đã xảy ra lỗi khi ghi nhận xe vào' });
    }
});
...
router.get('/', async (req, res) => {
    try {
        ...
        let query = `
            SELECT vi.*, v.owner_name, v.vehicle_type, b.name as building_name
            FROM vehicles_in vi
        ...
        query += ' ORDER BY vi.time_in DESC';
        
        const result = await pool.query(query, params);
        res.json(result.rows);
    } catch (err) {
        console.error('Lỗi lấy danh sách xe vào:', err);
        res.status(500).json({ message: 'Đã xảy ra lỗi khi lấy danh sách xe vào' });
    }
});
